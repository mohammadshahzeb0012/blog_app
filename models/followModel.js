...
const suggestionsUsersList = ({ SKIP, excludesUsers, followerUserId }) => {
  return new Promise(async (resolve, reject) => {
    try {
      const suggestionsUsers = await userSchema.aggregate([
        { $match: { _id: { $nin: excludesUsers } } },
        { $sort: { creationDateTime: -1 } },
        { $skip: SKIP },
        { $limit: LIMIT },
        { $project: { password: 0 } }
      ])
      const response = suggestionsUsers.filter(user => !user._id.equals(followerUserId))
      resolve(response)
    } catch (error) {
      reject(error)
    }
